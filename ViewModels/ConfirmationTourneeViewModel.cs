...
public sealed class ConfirmationTourneeViewModel : BaseViewModel
{
    ...
    public ConfirmationTourneeViewModel(AppStateService appStateService, TourneesApiService tourneesApiService, DatabaseService databaseService)
    {
    ...
    public string DateText => (_appStateService.SelectedTournee?.DateTournee ?? _appStateService.DateTourneeAutorisee ?? DateTime.Today).ToString("dd/MM/yyyy");

    public string TourneeText => _appStateService.SelectedTournee is null ? "Aucune tournée" : _appStateService.SelectedTournee.NomAffiche;
    ...
    private async Task LoadTourneeAsync()
    {
        ...
        try
        {
            ...
            if (expiredCount > 0)
            {
                LoadMessage = $"{expiredCount} ancienne(s) tournée(s) ont été marquée(s) EXPIREE et ne peuvent plus être reprises.";
            }
        ...
        {
            ErrorMessage = "Chargement impossible. La tournée sélectionnée n'a pas pu être récupérée depuis l'API. " + $"Détail : {exception.Message}";
            LoadMessage = ErrorMessage;
            await Shell.Current.CurrentPage.DisplayAlertAsync("Chargement impossible", ErrorMessage, "OK");
        }
        finally
        {
            SetBusy(false);
    ...
    private async Task<bool> HandleExistingActiveTourneeAsync(LocalTournee activeTournee, TourneeResumeDto selectedTournee)
    {
        var selectedLabel = string.IsNullOrWhiteSpace(selectedTournee.LibelleTournee) ? selectedTournee.CodeTournee : $"{selectedTournee.CodeTournee} — {selectedTournee.LibelleTournee}";
        var activeLabel = string.IsNullOrWhiteSpace(activeTournee.LibelleTournee) ? activeTournee.CodeTournee : $"{activeTournee.CodeTournee} — {activeTournee.LibelleTournee}";

        var action = await Shell.Current.CurrentPage.DisplayActionSheetAsync($"Une tournée non synchronisée existe déjà :\n{activeLabel} du {activeTournee.DateTournee:dd/MM/yyyy}\n\nQue voulez-vous faire ?", "Retour à la liste", $"Abandonner {activeTournee.CodeTournee} et charger {selectedTournee.CodeTournee}", "Reprendre cette tournée");
        ...
        if (string.Equals(action, $"Abandonner {activeTournee.CodeTournee} et charger {selectedTournee.CodeTournee}", StringComparison.OrdinalIgnoreCase))
        {
            var confirmed = await Shell.Current.CurrentPage.DisplayAlertAsync("Abandonner la tournée locale ?", $"Attention : cette action abandonnera la tournée locale non synchronisée {activeLabel} du {activeTournee.DateTournee:dd/MM/yyyy}.\n\nElle ne sera plus proposée à la reprise sur ce téléphone. Utilisez cette option uniquement si la tournée est bloquée ou chargée par erreur.\n\nVoulez-vous vraiment l'abandonner et charger {selectedLabel} ?", "Abandonner et charger", "Annuler");
            if (!confirmed)
            {
                return false;
            }

            await _databaseService.AbandonnerTourneeLocaleAsync(activeTournee.Id, $"Remplacée par la tournée {selectedTournee.CodeTournee} depuis l'écran de chargement.");
            await _databaseService.PurgeOldAbandonedTourneesAsync(retentionDays: 30);
    ...
    private async Task LoadSelectedTourneeFromApiAsync(TourneeResumeDto selectedTournee)
    {
        ...
        var dto = await _tourneesApiService.GetTourneeJourAsync(selectedTournee.CodeTournee, _appStateService.CurrentLivreur!.CodeLivreur);
        ...
        LoadingMessage = "Enregistrement local de la tournée...";
        LoadMessage = "Enregistrement local sur le téléphone…";
        await Task.Yield();
